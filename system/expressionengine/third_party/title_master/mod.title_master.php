...
class Title_master {
	...
	function _update_entry($entry_id,$title_tmpl,$url_title_tmpl,$channel_id){
	  ...
	  //If URL Title needs to be update, Do that
	  if($url_title_tmpl){
  		...
  		if($new_url != ""){
			...
			if($this->isStructurePage($channel_id)){
				...
				if($result->row()->update_structure == 1){
					...
					$this->updateStructureUrl($entry_id,$structure_url);
  ...
  function updateStructureUrl($entry_id,$new_url){
	...
	$last_segment = $total - 1;
	$segments[$last_segment] = $new_url;
