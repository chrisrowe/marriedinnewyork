...
class Sql_structure
{
	 ...
	 * @return array
	 */
	function get_selective_data($site_id, $current_id, $branch_entry_id, $mode, $show_depth, $max_depth, $status, $include, $exclude, $show_overview, $rename_overview, $show_expired, $show_future, $nav_hidden=false)
	{
	 ...
	 */
	function generate_nav($selective_data, $current_id, $entry_id, $mode,$show_overview,$rename_overview)
	{
		$html = '';
		$separator = $this->EE->config->item('word_separator') != "dash" ? '_' : '-';
		...
		for ($i=0; $i < $tree_count; $i++)
		{
			...
			{ 
				if ($this->EE->TMPL->fetch_param('rename_overview') == 'title' AND $custom_title_fields !== FALSE) 
				{ 
					$title = $custom_title_fields[$tree[$i]['entry_id']]; 
				}
				else
				{
			...
			if ($tree[$i]['depth'] < @$tree[$i+1]['depth'])
			{
				...
				if ($show_overview)
				{
					if ($tree[$i]['depth']=="1")
					{
					$html .= '<li'. $classes . $ids . '><a href="' . $tree[$i]['uri'] . '">'.$rename_overview.'</a>';	
					}
				}
